  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (index !== -1 && !pressedKeys.has(e.key)) {
        ...
        if (oscRef.current && envRef.current) {
          oscRef.current.freq(freq);
          envRef.current.play(); // starts attack -> decay -> sustain
        }
      }
    };
    ...
    const handleKeyUp = (e) => {
      if (pressedKeys.has(e.key)) {
        ...
        if (envRef.current) {
          // let release phase happen naturally (already configured in ADSR)
          // OR forcefully stop with .amp(0, release) if needed
        }
      }
    };
